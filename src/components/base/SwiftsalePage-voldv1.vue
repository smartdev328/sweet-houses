<template>

    <div class="page2">
      <div class="my-2 my-md-5 item12 mx-3">
        <div class="item12a DMSerifRegular text-center">
          <p>Swift Sale Benefits</p>
        </div>
        <div class="item12b p-4 mb-5" v-for="itembenefite in Benefitsdata" :key="itembenefite.id" style="background-color: #EDF3F2">
          <div class="benefits-card bg-white p-3">
            <div class="card my-2">
              <div class="row no-gutters">
                <div class="col-md-4">
                  <img :src="itembenefite.image" class="card-img" alt="...">
                </div>
                <div class="col-md-8">
                  <div class="card-body">
                    <h5 class="card-title DMSerifRegular">{{ itembenefite.title }}</h5>
                    <p class="card-text text-color-2 Roboto-Regular" v-html="itembenefite.content">
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>

        </div>
      </div>
      <div class="my-2 my-md-5 item7">
        <div class="text-center text-color-1 mb-5 DMSerifRegular text-capitalize">
          <p>What Our Customers Say</p>
        </div>

        <!-- <google-review></google-review> -->
        <div class="elfsight-app-3f411fcb-35bd-44e4-b1ce-551508d164d9"></div>
      </div>
      <div class="my-2 my-md-5 item13">
        <div class="text-center text-color-1 mb-5 DMSerifRegular text-capitalize">
          <p> A Swift Sale is Right For You, If You</p>
        </div>
        <div>
          <div class="image">
            <img class="rounded" src="../../assets/image/ASwiftSaleisRightForYou.png" alt="">
          </div>
          <div class="col-12 col-md-8  py-4 pl-4 my-4 mx-auto">
            <ul>
              <li v-for="item in Traditionaldata" :key="item.id" class="text-color-1 Roboto-Regular">
                <div class="icon mr-3">
                  <img src="../../assets/image/icon/checkicon.svg"  class="w-100 h-100" alt="">
                </div>
                {{ item.content }}
              </li>
            </ul>
          </div>
        </div>



      </div>
        <div class="bg-color-1 p-3 item1 DMSerifRegular">
                <div class="item1a text-center"><p class="text-color-1">How a Swift Sale Works</p></div>
                <div class="row d-flex align-items-center item1b">
                    <div class="col-12 col-md-8">
                 
                        <p class="Roboto-Medium">You choose the (flexible) moving day that works best for you. We pay the Appraisal value as determined by a bank-certified home appriaser and deduct our commission from the purchase price. Skip showings, fixups, weeks/months of being for sale, and all the seller pains that come from have a home listed for sale.</p>
                               <router-link to="Whats-My-Home-Worth" class="Roboto-Regular btn btn-route px-3">Start Now</router-link></div>
                    <div class="col-12 col-md-4">
                        <iframe width="100%" height="100%" src="https://www.youtube.com/embed/lm6lGa_4Ci0"  title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" style="background:#EDF3F2;border-radius:8px" allowfullscreen></iframe>
                    </div>
                </div>
        </div>
           <div class="my-2 my-md-5 item3new">
            <div class="cards">
                    <div class="card border-0">
                    <img src="../../assets/image/IpadwithForm.jpg" class="card-img-top" alt="...">
                    <div class="card-body pl-0">
                        <h3 class="card-title  DMSerifRegular">Step 1</h3>
                        <h3 class="card-title Roboto-Medium text-color-1">Tell Us About Your Property
</h3>
                        <p class="card-text text-color-2">Answer some easy questions about your home. You'll get a computer generated <a href="/Whats-My-Home-Worth">estimate</a> of the current value of your property.

</p>
                    </div>
                    </div>
                 
                 <div class="card border-0">
                    <img src="../../assets/image/YellowAppraisal.jpg" class="card-img-top" alt="...">
                    <div class="card-body pl-0">
                        <h3 class="card-title  DMSerifRegular">Step 2</h3>
                        <h3 class="card-title Roboto-Medium text-color-1">Appraisal & Inspection

</h3>
                        <p class="card-text text-color-2">
                            We will arrange for an independents, bank-certified appraisal and home inspection to establish our purchase price.
                        </p>
                    </div>
                    </div>
                  
                  <div class="card border-0">
                    <img src="../../assets/image/HappyFamilyMorning.jpg" class="card-img-top" alt="...">
                    <div class="card-body pl-0">
                        <h3 class="card-title  DMSerifRegular">Step 3</h3>
                        <h3 class="card-title Roboto-Medium text-color-1">Close & Move
</h3>
                        <p class="card-text text-color-2">Once we resell the improved home you get a second payment for the full resale price less our fee and our cost for improvements.</p>
                    </div>
                    </div>
            </div>
        </div>

           <div class="my-2 my-md-5 item11 mx-3">
            <div class="item11a DMSerifRegular text-center">
                <p>Why Choose a Swift Sale?</p>
            </div>
            <div class="item11b row p-0">
                <div class="element1 p-0 col-12 col-md-6">
                    <!-- <p class="p1 DMSerifRegular">Sweet Sale with Sweetly</p> -->
                    <img src="../../assets/image/wswift1.png" class="w-100 h-100" alt="">
                </div>
                <div class="element2 p-0 col-12 col-md-6">
                     <img src="../../assets/image/wswift2.png" class="w-100 h-100" alt="">
                </div>
            </div>
            <div>
                <p class="Roboto-Regular text-color-1 my-2">These comparisons are examples only. When listing traditionally market conditions and results will vary.  Commission is assumed to be 7% of $100,000 plus 3% balance.  A 3% concession to the asking price vs. sale price is average based on MLS data. 
                     Repairs necessary to complete a sale are property specific.</p>
            </div>
        </div>
        <div class="my-2 my-md-5 item3">
            <div class="row align-items-center ">
                <div class="col-12 col-md-6 bordered-left py-4 pl-4">
                    <p class="DMSerifRegular text-capitalize">Certainty</p>
                    <ul>
                        <li>
                            <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div> 
                       
                        A firm sale and your chosen moving date within days.

                       </li>
                        <li>  <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div>Skip all the seller pains of a home listed for sale.
</li>
                       <li>  <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div>Negotiate a better price and terms for your next home, knowing your current home is sold!
</li>
                    </ul>
                </div>
                <!-- <div class="col-12 col-md-1"></div> -->
                <div class="col-12 col-md-6 d-flex ">
                <div class="image">
                        <img src="../../assets/image/Component97-2.png" class="w-100 h-100" alt="">
                    </div>
                </div>
            </div>
        </div>
        <div class="my-2 my-md-5 item4">
            <div class="row align-items-center flex-row-reverse">
                <div class="col-12 col-md-6 d-flex">
                    <div class="image">
                        <img src="../../assets/image/homepage/Component 90 – 1.svg" class="w-100 h-100" alt="">
                    </div> 
                </div>
                <!-- <div class="col-12 col-md-1"></div> -->
                <div class="col-12 col-md-6 bordered-right py-4 pr-4">
                    <p class="DMSerifRegular text-capitalize">Skip ALL the Seller Pains</p>
                    <ul>
                        <li>
                            <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div> 
                            Never a need to tolerate public showings.
</li>
                        <li>  <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div>Never have unqualified, unidentified strangers invited into your home right off the street during an open house.
.</li>
                    <li>  <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div>Our purchase price is net to you, there are no hidden fees.
.</li>
                    </ul>
                </div>
            </div>
        </div>
         <div class="my-2 my-md-5 item5">
            <div class="row align-items-center">
                <div class="col-12 col-md-6 bordered-left py-4 pl-4">
                    <p class="DMSerifRegular text-capitalize">Choose your moving day</p>
                    <ul>
                        <li>
                            <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div> 
                        Choose the day that works best for you.
</li>
                        <li>  <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div>Change your moving day for free, without hassles.
.</li>
                       <li>  <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div>There's no negotiating price vs possession like a traditional sale
</li>
                    </ul>
                </div>
                <!-- <div class="col-12 col-md-1"></div> -->
                <div class="col-12 col-md-6 d-flex flex-row-reverse">
                     <div class="image">
                        <img src="../../assets/image/homepage/Component 84 – 1.svg" class="w-100 h-100" alt="">
                    </div> 
                </div>
            </div>
        </div>
        <div class="my-2 my-md-5 item6">
            <div class="row align-items-center flex-row-reverse">
                <div class="col-12 col-md-6 d-flex">
                     <div class="image">
                        <img  src="../../assets/image/homepage/Component 85 – 1.svg" class="w-100 h-100" alt="">
                    </div> 
                </div>
                <!-- <div class="col-12 col-md-1"></div> -->
                 <div class="col-12 col-md-6 bordered-right py-4 pr-4">
                    <p class="DMSerifRegular">Consider Any Other Offers</p>
                    <ul>
                        <li>
                            <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div> 
                            At your option, we'll list your home for sale until your 
possession day arrives. If you get a better offer from 
any other buyer, you can take it!.</li>
                        <li>  <div class="icon mr-3">
                                <img src="../../assets/image/homepage/icon2.svg"  class="w-100 h-100" alt="">
                            </div>There's no cost to accept a better offer - only our 
traditional Realtor commission will apply.</li>
                    </ul>
                </div>
            </div>
        </div>

    </div>
</template>
<script>

export default {
    data(){
        return{
            selected_part:"Swiftpart",
          Traditionaldata:[
            {content:" Value your time and convenience"},
            {content:"Have pets who don’t like stranger"},
            {content:"Have children and a busy lifestyle"},
            {content:" Like to be in control and hate surprises"},
            {content:"Already have a new home waiting"},
            {content:"Plan to buy your next home using the money from the one you have"},
          ],
          Benefitsdata:[
            {title:'Market Value Sale',
            content:` You get full market value on the day you choose.
  The cost is basically <u>the same as our standard full service commission</u>
  .`,
              image:require('../../assets/image/1-MarketValueSale.png')
            },
            {title:'Sell Without Listings ',
              content:` Accepting our offer means that your home isn’t listed for sale until after you move out.`,
              image:require('../../assets/image/2-SellWithoutListings.png')
            },
            {title:'No Showings',
              content:`Avoid the disruption of strangers touring your home and inconvenient open house weekends`,
              image:require('../../assets/image/HappyFamilyMorning.jpg')
            },
            {title:'Pick Your Closing Day',
              content:`Pick the moving day that works best for you (within 45 days). You may also change the day with notice at no cast.`,
              image:require('../../assets/image/4-PickYourClosingDay.jpeg')
            },

          ]
        }
    },
    computed:{
        swiftsalefaqs(){
            return this.$store.state.swiftsalefaqs
        }
    },
    methods:{
        openfaqspage(){
            this.$router.push({name:'FrequentlyQuestion'});
            window.scrollTo(0,0)
        }
    }
}
</script>

<style scoped>
    .page2 .item1 .item1a p{
        font-size: 36px;
    }
    .page2 .item1 .item1b p:first-child{
    color:#232323;
    font-size: 18px;
    }
    .page2 .item1 .item1b p:last-child{
        color:#434242;
        font-size: 18px;
    }
    .page2 .item2 .title p{
         font-size: 36px;
        font-weight: 600;
    }
    .page2 .item2 .item2a{
        width: 75%;
    }
    .page2 .item2 .item2a .item2b{
         font-size: 30px;
        cursor: pointer;
        font-weight: bold;
    }
    .page2 .item2 .item2a .item2b .active{
    background: rgb(255, 182, 0);margin-bottom: 0px;    width: 100%;  border-radius: 20px;height: 5px;
    }
    ul li{
        list-style-type: none;
        display: flex;
        font-size: 20px;
    }
    .item2c ul li .icon,
    .item2c2 ul li .icon{
        width: 24px;
        height: 24px;
        max-width: 24px;
        min-width: 24px;
    }
    .item2c .p1 , .item2c2 .p1{
    position: absolute;
    z-index: 10;
    left: 0;
    top: -2em;
    width: 253px;
    height: 302px;
    }
    .item2c .p2,
    .item2c2 .p2{
    position: absolute;
    z-index: 20;
    right: 0;
    top: 2em;
    width: 253px;
    height: 302px;
    }
    .item2c .gridcontainer,
    .item2c2 .gridcontainer{
        display: grid;
    grid-template-columns: auto auto auto auto auto auto auto;
    grid-gap: 10px;
    padding: 10px;
    }
    .item2c .gridcontainer .gridcol,
    .item2c2 .gridcontainer .gridcol
    {
        border-right: 1px solid #707070;
        font-family: "Roboto", sans-serif;
        position: relative;
        height: 94px;
    }
    .item2c .gridcontainer .gridcol:last-child,
    .item2c2 .gridcontainer .gridcol:last-child{
        border-right: 0;
    }
     .item2c .gridcontainer .gridcol p:first-child,
    .item2c2 .gridcontainer .gridcol p:first-child
    {
        font-family: 'Roboto-Regular' , sans-serif  !important;
        font-size: 18px;
    }
    .item2c .gridcontainer .gridcol p,
    .item2c2 .gridcontainer .gridcol p{
        font-family: 'Roboto-Medium',sans-serif  !important;
        color: #232323;
        font-size: 16px;
    /* position: absolute;
    left: 25%;
    right: 25%;
    top: 75%; */
    }
  /*   .item2c .gridcontainer .gridcol p:nth-child(2),
    .item2c2 .gridcontainer .gridcol p:nth-child(2){
    position: absolute;
    left: 25%;
    right: 25%;
    top: 75%;} */
    .item2c .lastitem,
    .item2c2 .lastitem{
        width: 60%;
        margin-top: unset;}
    .item2c .lastitem p,
    .item2c2 .lastitem p{
        font-size: 14px;
        
    }
    .page2 .item3 p ,
    .page2 .item5 p{
        font-size: 36px;
        width: 80%;
        font-weight: 600;
    }
    .page2 .item4 p ,
    .page2 .item6 p
    {
        font-size: 36px;
        width: 80%;
        font-weight: 600;
    }
    .page2 .item3 ul li,
    .page2 .item4 ul li,
    .page2 .item5 ul li,
    .page2 .item6 ul li,
    .page2 .item3 ul li{
        margin-bottom: 1em;
     /*    font-family: 'Roboto-Regular' !important; */
        font-family: 'Roboto', sans-serif;
    }
    .page2 .item3 ul li .icon ,
    .page2 .item5 ul li .icon{
        width: 18px;
        height: 18px;
        max-width: 18px;
        min-width: 18px;
    }
    .page2 .item4 ul li .icon,
    .page2 .item6 ul li .icon{
        width: 18px;
        height: 18px;
        max-width: 18px;
        min-width: 18px;
    }
    .page2 .item3 .image ,
    .page2 .item5 .image{
        width: 500px;
        height: 380px;
    }
    .page2 .item4 .image,
    .page2 .item6 .image{
        width: 500px;
        height: 380px;
    }
    .page2  .item7 .h-300px{
      height: 300px;
    }
    .page2 .item7 p{
      font-size: 38px;
      font-weight: 600;
    }
    .page2 .item8 .element1 p:first-child{
        font-size: 28px;
    }
    .page2 .item8 .element1 p:last-child{
        font-size: 16px;
    }
    .page2 .item8 .element2 table{
        padding: 8px 3px;
    }
    .page2 .item8 .element2 table tbody tr{
        padding: 12px 20px;
    }
    .page2 .item8 .element2 table tbody tr td:nth-child(2){
        background: #EDF3F2;
        padding: 5px 30px;
    }
    .page2 .item8 .element2 table tbody tr td:first-child{
        padding: 5px 20px;
    }
    .page2 .item8 .element2 table tbody tr:first-child td,
    .page2 .item8 .element2 table tbody tr:last-child td{
        padding: 24px 20px 8px;
    }
    .page2 .item8 .element2 table tbody tr:first-child td:nth-child(2){
        color: #FFB600;
        font-size: 24px !important;
    }
    .page2 .item8 .element2 table tbody tr td:nth-child(2) .h1,
    .page2 .item8 .element2 table tbody tr td:nth-child(2) .h1{
        font-size: 2.8em !important;
    }
    .page2 .item8 .element2 table tbody tr:nth-child(7) td:nth-child(2),
    .page2 .item8 .element2 table tbody tr:nth-child(8) td:nth-child(2){
        font-size: 16px !important;
    }
    .page2 .item8 .element4{
         font-size: 38px;
        font-weight: 600;
    }
    .page2 .item8 .element6 a{
        border-radius: 8px;
        border: 1px solid #AAAAAA;
        color: #707070;
        font-family: "Roboto", sans-serif;
        font-size: 18px;
        transition: 0.2s ease-in-out;
    }
    .page2 .item8 .element6 a:hover{
        border: 1px solid #FFB600;
        color: #FFB600;
    }
    .page2 .item2c2 ul{
            margin-top: unset;
    }
    .page2 .item2c  ul{
        margin-top: unset;
    }
        .page2 .item11 .item11a{
        font-size: 36px;
        font-weight: 600;
    }
    .page2 .item11 .item11b{
        border: 2px solid;
        background: rgb(229, 245, 244);
    }
    .page2 .item11 .item11b .element1{
            background: rgb(229, 245, 244);
            height: -webkit-fill-available;
              
    }
    .page2 .flex-row-reverse{
        flex-direction: row !important;
    }
       .page2 .item3new .cards{
         display: grid;
    grid-template-columns: auto auto auto ;
    grid-column-gap: 40px;
    grid-row-gap: 40px;
     }
     .page2 .item3new .cards .card img{
         border-radius: 8px;
     }
     .page2 .item3new .cards .card .card-body h3:first-child{
         color: #ffb600;
     }
      .page2 .btn-route{
         background: #ffb600;
            color: #fff;
            border-radius: 4px;
            font-size: 1.2em;
            translate: 0.3s ease-in-out;
     }
     .page2 .btn-route:hover{
         background: #ffb700d2;
     }
    .page2 .item12 .item12a{
      font-size: 36px;
      font-weight: 600;
    }
    .page2 .item12 .item12b,
    .page2 .item12 .item12b .benefits-card,
    .page2 .item12 .item12b .benefits-card .card img{
      border-radius: 8px;
    }
    .benefits-card .card{
      border: 0 !important;
    }
    .benefits-card .card .card-title{
      font-size: 32px;
    }
    .page2 .item13 p{
      font-size: 38px;
      font-weight: 600;
    }
    .page2 .item13 .image{
      width: 600px;
      margin: auto;
    }
    .item13 ul li{
      list-style-type: none;
      display: flex;
      font-size: 20px;
    }
    .item13 ul li .icon,
    .item13 ul li .icon{
      width: 22px;
      height: 22px;
      max-width: 24px;
      min-width: 24px;
    }
    .item13 ul li {
      font-size: 22px;
    }
    @media only screen and (max-width: 600px){
        .item2c .gridcontainer,
    .item2c2 .gridcontainer{
        display: grid;
    grid-template-columns: auto;}
    .item2c .gridcontainer .gridcol,
    .item2c2 .gridcontainer .gridcol{
        border-right: 0;
        border-bottom: 1px solid #707070;
    }
    .page2 .item2 .item2a{
        width:100%;
    }
    .page2 .item2 .title p{
        font-size: 20px;
    }
    .page2 .item2 .item2a .item2b{
        font-size: 12px;
    }
    .page2 .item2c2 ul{
            margin-top: 16em;
    }
    .page2 .item2c  ul{
       margin-top: 16em; 
    }
    .page2 ul li{
        font-size: 16px;
    }
    .item2c .lastitem,
    .item2c2 .lastitem{
        width: 90%;
        margin-top: 1.5rem;}
    .page2 .item3 p ,
    .page2 .item5 p{
        width: 100%;
        font-size: 28px;
        font-weight: 600;
    }
    .page2 .item4 p ,
    .page2 .item6 p{
        width: 100%;
        font-size: 28px;
        font-weight: 600;
    }
    .item2c .p2,
    .item2c2 .p2{
    position: absolute;
    z-index: 20;
    right: 0;
    top: 2em;
    width: 194px;
    height: 194px;
    }
    .item2c .p1 , .item2c2 .p1{
    position: absolute;
    z-index: 10;
    left: 0;
    top: -2em;
    width: 194px;
    height: 194px;
    }
    .page2 .item8 .element4{
        font-size: 32px;
    }
     .page2 .flex-row-reverse{
        flex-direction: column-reverse !important;
    }
    .page2 .item3new .cards{
         grid-template-columns: auto ;
     }
      .page2 .item7 p{
        font-size: 26px;
      }
    }

</style>
